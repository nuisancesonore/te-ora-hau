...
/* ---------- Bandeau de la version d'essai (dossier /v2/) ----------
   Cette copie sert a comparer la refonte avec le site en service.
   Elle partage la MEME base de donnees : ce qui y est enregistre est reel. */
function banniereVersionEssai() {
  if (document.getElementById("toh-essai")) return;
  const d = document.createElement("div");
  d.id = "toh-essai";
  d.style.cssText = "background:#8a4b00;color:#fff;padding:0.5rem 1rem;text-align:center;" +
    "font-size:0.85rem;line-height:1.45";
  d.innerHTML = "Version d'essai — refonte en cours. " +
    '<a href="../index.html" style="color:#ffd9a3;text-decoration:underline">Revenir au site en service</a>. ' +
    "<strong>Attention :</strong> cette version utilise la vraie base de données.";
  const mount = document.getElementById("nav-mount");
  if (mount) mount.parentNode.insertBefore(d, mount);
}
